...
<div class="container py-5">
    ...
    <div class="mb-5 reveal-up" style="transition-delay: 0.1s;">
        ...
        <div class="d-flex flex-wrap gap-4 text-muted fw-500 mb-4">
            <span class="d-flex align-items-center"><i class="bi bi-geo-alt fs-5 me-2 text-danger"></i> Khởi hành từ: {{ $tour->departure_location->name ?? 'Đang cập nhật' }}</span>
            <span class="d-flex align-items-center"><i class="bi bi-clock fs-5 me-2 text-warning"></i> {{ $tour->duration_days }} ngày {{ $tour->duration_nights }} đêm</span>
        </div>

        <div class="row g-3 mb-4">
            <div class="col-md-8 overflow-hidden-rounded">
                @php $primaryImage = $tour->tour_images->where('is_primary', 1)->first(); @endphp
                <img src="{{ $primaryImage ? $primaryImage->image_url : 'https://images.unsplash.com/photo-1501785888041-af3ef285b470?q=80&w=1200' }}" class="w-100 gallery-main" alt="Ảnh chính">
            </div>
            <div class="col-md-4 d-flex flex-column gap-3">
                ...
                @forelse($subImages as $img)
                    <div class="overflow-hidden-rounded h-50">
                        <img src="{{ $img->image_url }}" class="w-100 gallery-sub" alt="Ảnh phụ">
                    </div>
                @empty
                    <div class="overflow-hidden-rounded h-50"><img src="https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?q=80&w=600" class="w-100 gallery-sub" alt="Ảnh phụ"></div>
                    <div class="overflow-hidden-rounded h-50"><img src="https://images.unsplash.com/photo-1599839619722-39751411ea63?q=80&w=600" class="w-100 gallery-sub" alt="Ảnh phụ"></div>
                @endforelse
            </div>
        </div>
    </div>
    ...
    <div class="row g-5">
        <div class="col-lg-8">
            <div class="glass-panel p-4 p-md-5 mb-5 reveal-up">
                <h3 class="section-heading mb-4 text-dark fs-4">Tổng quan</h3>
                <p class="text-muted lh-lg fs-6">{!! nl2br(e($tour->description)) !!}</p>
            </div>

            <div class="glass-panel p-4 p-md-5 mb-5 reveal-up">
                <h3 class="section-heading mb-4 text-dark fs-4">Lịch trình chi tiết</h3>
                <div class="d-flex flex-column flex-md-row align-items-start gap-4">
                    <div class="nav flex-column nav-pills tour-nav-tabs w-100 w-md-25" id="itinerary-tabs" role="tablist" aria-orientation="vertical">
                        ...
                        <button class="nav-link mb-2 {{ $index == 0 ? 'active' : '' }}" data-bs-toggle="pill" data-bs-target="#day-{{ $itinerary->id }}" type="button" role="tab">
                            <div class="small text-uppercase mb-1" style="opacity: 0.8; letter-spacing: 1px;">Ngày {{ $itinerary->day_number }}</div>
            ...
            <div class="glass-panel p-4 p-md-5 mb-5 reveal-up">
                <h3 class="section-heading mb-4 text-dark fs-4">Các hoạt động nổi bật</h3>
                                ...
                                <div class="col-md-6">
                                    <div class="premium-card h-100">
                                        <div class="card-img-wrapper" style="height: 180px;">
                                            <img src="{{ $activity->image_url ?? 'https://images.unsplash.com/photo-1542314831-c6a4d142104d?q=80&w=400' }}" class="card-img-top" alt="Ảnh hoạt động">
                                        </div>
        ...
        <div class="col-lg-4">
            <div class="premium-card booking-sidebar p-4 p-md-5 reveal-up">
                <div class="d-flex align-items-center mb-4 pb-3 border-bottom">
                    <div class="fs-1 fw-bold text-secondary">{{ number_format($tour->base_price, 0, ',', '.') }}<span class="fs-5 text-muted fw-normal ms-1">₫</span></div>
                    <span class="ms-2 text-muted">/ khách</span>
                </div>

                <h5 class="mb-4 fw-bold text-dark fs-5">Chọn lịch trình khởi hành</h5>
                ...
                <form action="{{ route('frontend.tours.checkout') }}" method="POST">
                    ...
                    <div class="row g-3 mb-4 pb-4 border-bottom">
                        <div class="col-6">
                            <label class="form-label mb-2 fw-600 text-dark">Người lớn</label>
                        ...
                        <div class="col-6">
                            <label class="form-label mb-2 fw-600 text-dark">Trẻ em</label>
                            <div class="input-group">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <span class="text-muted fw-600">Tổng cộng:</span>
                        <span class="fs-3 fw-bold text-danger"><span id="totalPrice">{{ number_format($tour->base_price, 0, ',', '.') }}</span>₫</span>
                    </div>
